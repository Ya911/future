...
function Calculator1({frist,scend,thre,fored , id}) {
    ...
    const caluc = `${id === 0 ? "text-black  bg-slate-400" : 'bg-gray-600 text-white'}  w-16 h-16 text-center text-2xl rounded-full shadow-md cursor-pointer hover:bg-[#b5b5b5]`
    ...
    return (
      <div className={`flex justify-center  font-fonten gap-3`}>
        <button 
        onClick={(e)=>dCNum(e)}  
        className={id===4 ? "text-center bg-gray-600 flex-grow  w-28 h-16 text-2xl text-white rounded-[2rem] shadow-md cursor-pointer hover:bg-[#b5b5b5]" : caluc}>
